...
| `POST /auth/coe-governance/request-code` | Any signed-in user. Emails a fresh 6-digit code to the caller's own address (purpose `coe_toggle`), to be submitted to one of the two `coe-governance` endpoints above. Not scoped to a workspace/project — it only proves "signed in as this email, right now"; the confirm endpoints check admin permission on the specific resource separately. Same rate limiting shape (60s cooldown, 5/day) as `POST /auth/request-code`, but its own independent per-email budget, keyed by `purpose` — using up (or having recently used) your sign-in-code allowance never blocks a governance toggle, and toggling a second project right after the first doesn't 429 off its own prior request. |
